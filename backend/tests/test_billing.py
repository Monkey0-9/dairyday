"""
Billing service tests for DairyOS.
Tests bill calculation and generation logic.
"""

import pytest
from decimal import Decimal
from datetime import date
from uuid import uuid4

from app.models.user import User
from app.models.consumption import Consumption
from app.models.bill import Bill
from app.services.billing_service import BillingService
from app.core.money import calculate_amount


def test_calculate_month_range():
    """Test month range calculation for January 2026."""
    start, end = BillingService.calculate_month_range("2026-01")
    assert start == date(2026, 1, 1)
    assert end == date(2026, 1, 31)
    
    # February (non-leap year)
    start, end = BillingService.calculate_month_range("2026-02")
    assert start == date(2026, 2, 1)
    assert end == date(2026, 2, 28)
    
    # February (leap year)
    start, end = BillingService.calculate_month_range("2024-02")
    assert start == date(2024, 2, 1)
    assert end == date(2024, 2, 29)


def test_billing_calculation():
    """Test bill calculation accuracy."""
    # Test with user having price_per_liter = 50
    price_per_liter = Decimal("50.00")
    total_liters = Decimal("10.5")
    
    # Check money logic directly since calculate_bill_amount isn't exposed
    money = calculate_amount(total_liters, price_per_liter)
    expected = Decimal("525.00")  # 10.5 * 50
    
    assert money.amount == expected


@pytest.mark.asyncio
async def test_bill_total_calculation(db_session):
    """Test that bill total equals liters * price."""
    billing_service = BillingService(db_session)
    
    # Create test user
    user = User(
        id=uuid4(),
        email="test_bill_total@example.com",
        name="Test Bill Total User",
        logto_id="test_bill_total", # Unique constraint
        role="USER",
        price_per_liter=Decimal("60.00"),
        is_active=True
    )
    db_session.add(user)
    await db_session.flush()
    
    # Add consumption: 10L for the user in January 2026
    for i in range(10):
        consumption = Consumption(
            id=uuid4(),
            user_id=user.id,
            date=date(2026, 1, i+1),
            quantity=Decimal("1.0"),
            source="TEST"
        )
        db_session.add(consumption)
    await db_session.commit()
    
    # Generate bill for January 2026
    bill = await billing_service.generate_bill_for_user(user.id, "2026-01", enqueue_pdf=False)
    
    assert bill is not None
    assert bill.total_liters == Decimal("10.000") # Expect 3 decimals
    assert bill.total_amount == Decimal("600.00")  # 10 * 60


@pytest.mark.asyncio
async def test_bill_regeneration_no_duplicates(db_session):
    """Test that regenerating a bill doesn't create duplicates."""
    billing_service = BillingService(db_session)
    from sqlalchemy import select, func
    
    # Create test user
    user = User(
        id=uuid4(),
        email="test_regeneration@example.com",
        name="Test Regeneration User",
        logto_id="test_regeneration", # Unique constraint
        role="USER",
        price_per_liter=Decimal("50.00"),
        is_active=True
    )
    db_session.add(user)
    await db_session.flush()

    # Add dummy consumption
    consumption = Consumption(
        id=uuid4(),
        user_id=user.id,
        date=date(2026, 2, 15),
        quantity=Decimal("10.0"),
        source="TEST"
    )
    db_session.add(consumption)
    await db_session.commit()
    
    # Generate bill first time
    bill1 = await billing_service.generate_bill_for_user(user.id, "2026-02", enqueue_pdf=False)
    
    # Generate bill again (should update, not create new)
    bill2 = await billing_service.generate_bill_for_user(user.id, "2026-02", enqueue_pdf=False)
    
    # Check only one bill exists
    result = await db_session.execute(
        select(func.count(Bill.id)).where(
            Bill.user_id == user.id,
            Bill.month == "2026-02"
        )
    )
    count = result.scalar()
    
    assert count == 1
    assert bill1.id == bill2.id  # Same bill updated

