...
@pytest.mark.asyncio
async def test_full_flow(client: AsyncClient, db_session: AsyncSession):
    ...
    login_res = await client.post(
        "/api/v1/auth/login", 
        data={"username": "flow_test_admin@dairy.com", "password": "admin123"}
    )
    ...
    login_res = await client.post(
        "/api/v1/auth/login",
        data={"username": "flow_test_user@dairy.com", "password": "user123"}
    )
    assert login_res.status_code == 200
    user_tokens = login_res.json()
    ...
    response = await client.patch(
        "/api/v1/consumption/", json=consumption_data, headers=admin_headers
    )
    ...
    generate_headers = admin_headers.copy()
    generate_headers["Idempotency-Key"] = str(uuid.uuid4())
    ...
    bill_resp = await client.get(
        f"/api/v1/bills/{regular_user.id}/{month_str}", headers=admin_headers
    )
    ...
    response = await client.get(
        f"/api/v1/bills/{regular_user.id}/{month_str}", headers=user_headers
    )
    ...
    admin_consumption = Consumption(
        id=uuid.uuid4(), user_id=admin.id, date=yesterday, quantity=Decimal("5.0")
    )
    db_session.add(admin_consumption)
    await db_session.commit()

    response = await client.post(
        f"/api/v1/bills/generate/{admin.id}/{month_str}", headers=admin_headers
    )

    # User tries to access admin's bill - should be forbidden
    response = await client.get(
        f"/api/v1/bills/{admin.id}/{month_str}", headers=user_headers
    )
    assert response.status_code == 403, (
        f"Should have forbidden access to admin's bill: {response.text}"
    )

    # 8. User can view own consumption history
    response = await client.get(
        f"/api/v1/consumption/mine?month={month_str}", headers=user_headers
    )
    ...
    assert response.status_code == 403, (
        f"Regular user should not access admin endpoints: {response.text}"
    )

    # 11. Refresh token works
    response = await client.post(
        "/api/v1/auth/refresh", params={"refresh_token": user_tokens["refresh_token"]}
    )
    assert response.status_code == 200
    ...
    mark_paid_headers = admin_headers.copy()
    mark_paid_headers["Idempotency-Key"] = str(uuid.uuid4())
    response = await client.post(
        f"/api/v1/payments/mark-paid/{bill_id}", headers=mark_paid_headers
    )
...
@pytest.mark.asyncio
async def test_lock_rule_enforcement(client: AsyncClient, db_session: AsyncSession):
    ...
    login_res = await client.post(
        "/api/v1/auth/login",
        data={
            "username": "lock_test_admin@dairy.com",
            "password": "admin123",
        },
    )
    assert login_res.status_code == 200
    ...
    response = await client.patch(
        "/api/v1/consumption/", json=consumption_data, headers=headers
    )
    assert response.status_code == 400, (
        f"Should reject old consumption: {response.text}"
    )
    assert (
        "locked" in response.json()["detail"].lower()
        or "older" in response.json()["detail"].lower()
    )
    ...
    assert response.status_code == 200, (
        f"Should accept today's consumption: {response.text}"
    )
...
@pytest.mark.asyncio
async def test_user_data_isolation(client: AsyncClient, db_session: AsyncSession):
    ...
    login_res = await client.post(
        "/api/v1/auth/login",
        data={
            "username": "user1_isolation@dairy.com",
            "password": "user123",
        },
    )
    assert login_res.status_code == 200
    user1_token = login_res.json()["access_token"]
    ...
    login_res = await client.post(
        "/api/v1/auth/login",
        data={
            "username": "user2_isolation@dairy.com",
            "password": "user123",
        },
    )
    assert login_res.status_code == 200
    ...
    response = await client.get(
        f"/api/v1/bills/{user2.id}/{current_month}", headers=user1_headers
    )
    assert response.status_code == 403

    # User2 cannot access User1's bills
    response = await client.get(
        f"/api/v1/bills/{user1.id}/{current_month}", headers=user2_headers
    )
    assert response.status_code == 403
