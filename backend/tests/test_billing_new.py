...
from app.core.money import calculate_amount, format_currency
...
class TestCalculateMonthRange:
    ...
    def test_valid_months(self):
        ...
        # January
        start, end = BillingService.calculate_month_range("2026-01")
        assert start == date(2026, 1, 1)
        assert end == date(2026, 1, 31)
        
        # February (non-leap year)
        start, end = BillingService.calculate_month_range("2026-02")
        assert start == date(2026, 2, 1)
        assert end == date(2026, 2, 28)
        
        # February (leap year)
        start, end = BillingService.calculate_month_range("2024-02")
        assert start == date(2024, 2, 1)
        assert end == date(2024, 2, 29)
        
        # December
        start, end = BillingService.calculate_month_range("2026-12")
    ...
    def test_invalid_month_format(self):
        ...
        with pytest.raises(ValueError, match="Invalid month format"):
            BillingService.calculate_month_range("2026/01")
        
        with pytest.raises(ValueError, match="Invalid month format"):
            BillingService.calculate_month_range("jan-2026")
        
        with pytest.raises(ValueError, match="Invalid month format"):
            BillingService.calculate_month_range("202601")
    
    def test_invalid_month_number(self):
        """Test that invalid month numbers raise ValueError."""
        with pytest.raises(ValueError):
            BillingService.calculate_month_range("2026-00")
        
        with pytest.raises(ValueError):
            BillingService.calculate_month_range("2026-13")
...
class TestCalculateBillAmount:
    ...
    def test_basic_calculation(self):
        ...
        amount = calculate_amount(total_liters, price_per_liter).amount
    ...
    def test_rounding_behavior(self):
        ...
        # 0.1 * 0.3 = 0.03 - exact
        assert calculate_amount(Decimal("0.1"), Decimal("0.3")).amount == Decimal("0.03")
        
        # 0.1 * 0.25 = 0.025 - should round to 0.03 (Standard rounding)
        assert calculate_amount(Decimal("0.1"), Decimal("0.25")).amount == Decimal("0.03")
    
    def test_zero_liters(self):
        """Test with zero consumption."""
        amount = calculate_amount(Decimal("0"), Decimal("50.00")).amount
        assert amount == Decimal("0.00")
    
    def test_zero_price(self):
        """Test with zero price per liter."""
        amount = calculate_amount(Decimal("100"), Decimal("0")).amount
        assert amount == Decimal("0.00")
    ...
    def test_large_amount(self):
        ...
        amount = calculate_amount(total_liters, price).amount
...
class TestGenerateBillForUser:
    ...
    async def test_bill_generation_with_consumption(self, db_session):
        ...
        with patch('app.workers.tasks.generate_and_upload_pdf') as mock_task:
            ...
            bill = await BillingService(db_session).generate_bill_for_user( 
                user.id, 
                "2026-01",
                enqueue_pdf=False
            )
