...
async def test_webhook_marks_bill_paid(client, auth_headers):
    ...
    headers_with_idemp = auth_headers.copy()
    headers_with_idemp["Idempotency-Key"] = str(uuid.uuid4())
    await client.post(
        f"/api/v1/bills/generate-all?month={month}", headers=headers_with_idemp
    )
    ...
    body = {
        ...
        "payload": {
            "payment": {"entity": {"id": payment_id, "notes": {"bill_id": None}}}
        },
    }
