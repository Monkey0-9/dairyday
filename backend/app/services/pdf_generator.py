...
def generate_invoice_pdf(
    user: User, bill: Bill, consumptions: List[Consumption]
) -> BytesIO:
    buffer = BytesIO()
    ...
    info_data = [
        [
            Paragraph("CUSTOMER DETAILS", label_style),
            Paragraph("INVOICE DETAILS", label_style),
        ],
        [
            Paragraph(user.name, value_style),
            Paragraph(f"Invoice ID: {str(bill.id)[:8]}...", value_style),
        ],
        [
            Paragraph(f"Phone: {user.phone or 'N/A'}", label_style),
            Paragraph(f"Issued On: {date_type.today()}", label_style),
        ],
        [
            Paragraph(f"Email: {user.email or 'N/A'}", label_style),
            Paragraph(f"Status: {bill.status}", value_style),
        ],
    ]
    ...
    data.append(
        [
            ...
            "",
            "",
            Paragraph("TOTAL AMOUNT", value_style),
            Paragraph(f"₹{bill.total_amount:.2f}", value_style),
        ]
    )
        ...
        TableStyle(
            [
                ...
                (
                    ...
                    (-1, -1),
                    "RIGHT",
                ),  # Right align quantity and amounts
    ...
    elements.append(
        Paragraph(
            "This is a computer generated invoice. No signature required.", footer_style
        )
    )
...
def generate_consumption_report_pdf(
    user_name: str, month: str, data: List[dict], is_admin: bool = False
) -> BytesIO:
    """
    ...
    if is_admin:
        ...
        table_data.append(
            [
                ...
                Paragraph(f"{grand_total:.1f} L", label_style),
            ]
        )
        ...
        for key, val in user_row.items():
            if key.isdigit():
                if float(val) > 0:
                    table_data.append(
                        [f"Day {key}", "Fresh Milk Delivery", f"{float(val):.1f} L"]
                    )

        table_data.append(
            [
                "",
                Paragraph("TOTAL CONSUMPTION", label_style),
                Paragraph(f"{total:.1f} L", label_style),
            ]
        )
    ...
    elements.append(
        Paragraph(
            f"Generated on: {date_type.today().strftime('%B %d, %Y')}", footer_style
        )
    )
    elements.append(
        Paragraph("DairyDay Enterprise - Pure Milk, Pure Trust.", footer_style)
    )
