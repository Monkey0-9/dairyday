...
class ConsumptionService:
    ...
    async def get_grid_data(self, month: str) -> List[dict]:
        ...
        audit_map = {}
        for a in audits:
            if a.user_id not in audit_map: audit_map[a.user_id] = {}
        ...
        for c in consumptions:
            if c.user_id not in cons_map: cons_map[c.user_id] = {}
            cons_map[c.user_id][c.date] = float(c.quantity)
            if c.request_status == 'PENDING':
                if c.user_id not in request_map: request_map[c.user_id] = {}
        ...
        for user in users:
            ...
            for d in range(1, last_day + 1):
                ...
                row["daily_liters"][date_str] = cons_map.get(user.id, {}).get(curr_date, 0.0)
    ...
    async def upsert_admin(
        self, user_id: UUID, date_val: datetime.date, 
        quantity: float, extra_qty: float, 
        ...
        if existing:
            ...
            audit = ConsumptionAudit(
                user_id=user_id, admin_id=admin_id, date=date_val,
            ...
            new_c = Consumption(
                ...
                quantity=quantity, extra_qty=extra_qty,
