...
class Bill(Base):
    ...
    # UTR Submission for manual payments
    ...
    original_bill = relationship(
        "Bill", remote_side=[id], foreign_keys=[original_bill_id]
    )
    ...
    __table_args__ = (
        ...
        Index("idx_bills_utr_reference", "utr_reference"),
        CheckConstraint("status IN ('PAID', 'UNPAID', 'OVERDUE')", name="check_bill_status"),
    )
