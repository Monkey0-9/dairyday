...
class ConsumptionArchive(Base):
    ...
    original_consumption_id = Column(
        UUID(as_uuid=True),
        ForeignKey("consumption.id", ondelete="SET NULL"),
        nullable=True,
    )
    user_id = Column(
    ...
    archived_payload = Column(
        JSON, nullable=True
    )  # Full JSON backup of original consumption + audit rows
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationships
    original_consumption = relationship(
        "Consumption", foreign_keys=[original_consumption_id]
    )
