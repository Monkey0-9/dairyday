...
class ConsumptionBase(BaseModel):
    ...
    date: datetime.date = Field(..., description="Date of milk consumption")
    quantity: Decimal = Field(
        ..., ge=0, le=1000, description="Quantity of milk in liters"
    )
    ...
    note: Optional[str] = Field(None, description="Optional note for the delivery")
...
class ConsumptionUpdate(BaseModel):
    ...
    quantity: Optional[Decimal] = Field(
        None, ge=0, le=1000, description="Updated quantity of milk in liters"
    )
...
class ConsumptionInDBBase(ConsumptionBase):
    ...
    user_id: UUID
    locked: bool = Field(
        default=False, description="Whether the entry is locked for editing"
    )
