"""
Comprehensive E2E test for ALL billing and payment endpoints.
Writes results to test_output.txt (UTF-8).
"""
import sys
import asyncio
import traceback
...
async def main():
    ...
    try:
        async with httpx.AsyncClient(
            ...
            record("Login POST", r.status_code == 200,
    ...
    async with SessionLocal() as db:
        ...
        if test_user:
            ...
            record("Test user", True,
                   f"{test_user.name} ({test_user.email})")
        else:
            result = await db.execute(
            ...
            if any_u:
                ...
                record("Test user (fallback)", True,
                       f"{any_u.name}")
            else:
        ...
        result = await db.execute(
            select(Consumption).where(and_(
                Consumption.user_id == test_user_id,
                Consumption.date >= start
            ))
        )
        existing = result.scalars().all()
        if existing:
            record("Consumption data", True,
    ...
    if token:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                r = await client.post(
                    f"/api/v1/bills/generate/"
                    f"{test_user_id}/{test_month}"
                )
                record("POST /bills/generate/user/month",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
                if r.status_code == 200:
                    data = r.json()
                ...
                record("POST generate bill", False,
                       str(e)[:80])

            # Generate all bills
            try:
                r = await client.post(
                    f"/api/v1/bills/generate-all"
                    f"?month={test_month}"
                )
                record("POST /bills/generate-all",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
                if r.status_code == 200:
                    log(f"    {r.json()}")
                ...
                record("POST generate-all", False,
                       str(e)[:80])
            ...
            try:
                r = await client.get(
                    f"/api/v1/bills/?month={test_month}"
                )
                record("GET /bills/ (list)",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
                if r.status_code == 200:
                    log(f"    count={len(r.json())}")
            except Exception as e:
                record("GET bills list", False,
                       str(e)[:80])
            ...
            if test_bill_id:
                try:
                    r = await client.get(
                        f"/api/v1/bills/"
                        f"{test_bill_id}/pdf-status"
                    )
                    record("GET pdf-status",
                           r.status_code == 200,
                           f"HTTP {r.status_code}")
                except Exception as e:
                    record("GET pdf-status", False,
                           str(e)[:80])
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                record("GET /payments/last",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
            except Exception as e:
                record("GET /payments/last", False,
                       str(e)[:80])
            ...
            if test_bill_id:
                try:
                    ...
                    r = await client.post(
                        f"/api/v1/payments/"
                        f"create-order/{test_bill_id}"
                    )
                    ok = r.status_code in (200, 500)
                    record("POST create-order", ok,
                           f"HTTP {r.status_code}"
                           f" (500=no Razorpay)")
                except Exception as e:
                    record("POST create-order", False,
                           str(e)[:80])
            ...
            if test_bill_id:
                try:
                    ...
                    r = await client.post(
                        f"/api/v1/payments/"
                        f"mark-paid/{test_bill_id}"
                    )
                    record("POST mark-paid",
                           r.status_code == 200,
                           f"HTTP {r.status_code}")
                    if r.status_code != 200:
                        log(f"    ERR: {r.text[:150]}")
                except Exception as e:
                    record("POST mark-paid", False,
                           str(e)[:80])
            ...
            if test_bill_id:
                try:
                    async with SessionLocal() as db:
                        ...
                        bill = res.scalars().first()
                        record("Bill PAID after mark-paid",
                               bill and bill.status == "PAID",
                               f"{bill.status if bill else 'N/A'}")

                        pay = await db.execute(
                            select(Payment).where(
                                Payment.bill_id == test_bill_id
                            )
                        )
                        p = pay.scalars().first()
                        record("Payment record exists",
                               p is not None,
                               f"Provider={p.provider if p else 'N/A'}")
                except Exception as e:
                    record("Post-payment verify", False,
                           str(e)[:80])
        ...
        async with httpx.AsyncClient(
            ...
            try:
                r = await client.get(
                    f"/api/v1/consumption/"
                    f"grid?month={test_month}"
                )
                record("GET /consumption/grid",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
            except Exception as e:
                record("GET /consumption/grid", False,
                       str(e)[:80])

            try:
                r = await client.get(
                    f"/api/v1/consumption/"
                    f"mine?month={test_month}"
                )
                record("GET /consumption/mine",
                       r.status_code == 200,
                       f"HTTP {r.status_code}")
            except Exception as e:
                record("GET /consumption/mine", False,
                       str(e)[:80])
    ...
    try:
        async with SessionLocal() as db:
            ...
            bill = await BillingService.recalculate_bill(
                db, test_user_id, test_month
            )
            record("BillingService.recalculate",
                   bill is not None,
                   f"Amt={bill.total_amount if bill else None}")
    except Exception as e:
        record("BillingService.recalculate", False,
               str(e)[:100])
    ...
    try:
        async with SessionLocal() as db:
            ...
            svc = BillService(db)
            b = await svc.bill_repo.get_by_user_and_month(
                test_user_id, test_month
            )
            if b:
                bp = await svc.get_bill_with_pdf(b.id)
                record("BillService.get_bill_with_pdf",
                       bp is not None, "OK")
            else:
                record("BillService.get_bill_with_pdf",
                       True, "No bill (OK)")
    ...
    try:
        async with SessionLocal() as db:
            ...
            r = await svc.verify_webhook_signature(
                b"test", "fake"
            )
            record("PaymentService.verify_sig", True,
                   f"Result={r}")
    except Exception as e:
    ...
    try:
        async with SessionLocal() as db:
            ...
            b = await repo.get_by_user_and_month(
                test_user_id, test_month
            )
            record("BillRepo.get_by_user_month",
                   True, f"Found={b is not None}")
            bs = await repo.get_all_for_month(test_month)
            record("BillRepo.get_all_for_month",
                   isinstance(bs, list), f"Count={len(bs)}")
            p = await repo.get_pending_for_user(test_user_id)
            record("BillRepo.get_pending",
                   isinstance(p, list), f"Count={len(p)}")
    except Exception as e:
    ...
    try:
        async with SessionLocal() as db:
            ...
            if test_bill_id:
                ...
                record("PaymentRepo.get_all_for_bill",
                       isinstance(ps, list),
                       f"Count={len(ps)}")
            wh = await repo.get_webhook_event("nonexistent")
            record("PaymentRepo.get_webhook_event",
                   wh is None, "None (correct)")
    except Exception as e:
    ...
    try:
        async with SessionLocal() as db:
            ...
            record("ConsumptionRepo.stats", True,
    ...
    try:
        async with SessionLocal() as db:
            ...
            record("UserRepo.get_active_users",
                   isinstance(us, list),
                   f"Count={len(us)}")
    except Exception as e:
