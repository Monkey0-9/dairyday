...
const GlassStat = ({ title, value, icon, gradient, loading }: GlassStatProps) => (
  <motion.div
    ...
    className="glass-card rounded-xl p-3 border-white/5 bg-white/[0.02] hover:border-white/20 transition-all duration-700 flex flex-col justify-between relative overflow-hidden group h-[100px]"
  >
    <div className={cn("absolute inset-0 opacity-5 bg-gradient-to-br", gradient)} />
    <div className="flex justify-between items-start relative z-10">
      <div className="h-7 w-7 rounded-lg flex items-center justify-center border border-white/10 bg-white/5 text-white/40 group-hover:text-primary group-hover:border-primary/20 transition-all duration-700">
        {React.cloneElement(icon as React.ReactElement, { size: 14, "aria-hidden": "true" })}
      </div>
      <p className="font-micro text-[7px] font-black text-white/20 uppercase tracking-[0.3em] italic">{title}</p>
    </div>
...
export default function CustomersPage() {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<{ id: string; name?: string; email?: string; phone?: string; role?: string; is_active?: boolean; price_per_liter?: number; daily_target_qty?: number } | null>(null)
  ...
  if (isError) {
    return (
      <div className="container mx-auto px-4 py-20">
        <PremiumErrorState 
          message={t('registryLoadError') || "Failed to load customer registry"} 
          onRetry={() => refetch()} 
        />
      </div>
    )
    ...
    <div className="bg-transparent text-white selection:bg-primary/40 space-y-6 pb-12 relative">
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-3 border-b border-white/[0.03] pb-4 relative z-10">
        <div className="space-y-0.5">
          ...
          >
            <ShieldCheck className="h-1.5 w-1.5 text-primary" aria-hidden="true" />
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2">
          <div className="flex items-center gap-1 p-0.5 bg-obsidian-700/40 border border-white/5 rounded-lg shadow-glass-elev backdrop-blur-3xl">
            <Button variant="ghost" size="icon" className="h-7 w-7 text-white/40 hover:text-white" onClick={handlePrevMonth} aria-label={tCommon('accessibility.previousMonth')}>
            ...
            <Button variant="ghost" size="icon" className="h-7 w-7 text-white/40 hover:text-white" onClick={handleNextMonth} aria-label={tCommon('accessibility.nextMonth')}>
          ...
          <Button
            ...
            aria-label={t('addCustomer').toUpperCase()}
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
        ...
        <GlassStat
          title={t('avgRate')}
          value={`₹${formatCurrency(parseFloat(stats.avgRate), locale)}`}
      ...
      <section className="space-y-4 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-obsidian-800/40 p-3 rounded-2xl border border-white/[0.03] backdrop-blur-3xl shadow-glass-elev relative overflow-hidden group">
          ...
          <div className="relative flex-1 w-full z-10">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20" size={14} aria-hidden="true" />
            <Input
        ...
        <div className="rounded-xl border border-white/[0.03] bg-obsidian-800/40 backdrop-blur-3xl shadow-glass-elev overflow-x-auto group/table relative mask-edge-fade-right">
          <Scanline />
          <Table className="relative z-10 min-w-[700px]">
            <TableHeader>
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="h-96 text-center border-none p-0">
                      <EmptyUserState onAdd={() => setIsCreateOpen(true)} />
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    >
                      <TableCell className="py-2.5 px-4 lg:px-6">
                        <div className="flex items-center gap-2 lg:gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] font-bold text-white/70 tracking-wide font-mono mt-0.5">{customer.email || customer.id?.split('-')[0].toUpperCase()}</p>
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right py-2.5 px-4">
                        <span className="text-base lg:text-lg font-black font-heading italic tracking-tighter text-gradient">
                          ₹{formatCurrency(Number(customer.price_per_liter || 60), locale)}
                        </span>
                      </TableCell>
                      <TableCell className="text-center py-2.5 px-4">
                        <Badge className={cn(
                          ...
                          {customer.is_active ? <UserCheck size={10} aria-hidden="true" /> : <UserX size={10} aria-hidden="true" />}
                          {tCommon(`status.${customer.is_active ? 'ACTIVE' : 'INACTIVE'}`)}
                        </Badge>
                      </TableCell>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-7 w-7 rounded-lg bg-white/[0.02] border border-white/5 hover:bg-white/5 opacity-40 group-hover:opacity-100 transition-all"
                              aria-label={tCommon('accessibility.openMenu')}
                            >
                              <Fingerprint className="text-primary" size={14} aria-hidden="true" />
                            </Button>
                          </DropdownMenuTrigger>
...
function CreateCustomerDialog({ open, onOpenChange, onSuccess }: { open: boolean; onOpenChange: (open: boolean) => void; onSuccess: () => void }) {
  ...
  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", price_per_liter: "60", daily_target_qty: "1.0", password: "", confirmPassword: ""
  })
...
function EditCustomerDialog({ customer, open, onOpenChange, onSuccess }: { customer: { id: string; name?: string; email?: string; phone?: string; price_per_liter?: number; daily_target_qty?: number; subscription_plan?: string }; open: boolean; onOpenChange: (open: boolean) => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await usersApi.update(customer.id, updatePayload)
    ...
    >
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <Label className="text-[9px] font-black uppercase tracking-widest text-white/20 italic">{t('passwordLabel')} (Reset)</Label>
              <div className="relative">
                <Input
                  className="h-9 bg-white/[0.02] border-white/5 rounded-xl font-bold italic text-[13px] pr-8"
                  type={showPassword ? "text" : "password"}
